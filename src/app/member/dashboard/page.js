...
import CommunicationsSection from "@/app/admin/dashboard/components/CommunicationsSection";
...
export default function MemberDashboard() {
  ...
  const { dynamicGeneralFiles, staticMembershipDocs } = useMemo(() => {
    const dynamicGeneralFiles =
      files.generalFiles.find((c) => c.category === "Pliki Ogólne")?.files ||
      [];
    const staticDocs = files.generalFiles.find(
      (c) => c.category === "Dokumenty członkowskie (statuty, regulaminy)",
    );
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      {session?.user?.mustChangePassword && <ForceChangePasswordModal />}
                  ...
                  <div className="divide-y divide-gray-200">
                    ...
                    {files.generatedAcceptanceDocs.length > 0 && (
                      <CollapsibleFileCategory
                        category={generatedDocsCategory}
                      />
                    )}
                  </div>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
                  <div className="divide-y divide-gray-200">
                    ...
                    {dynamicGeneralFiles.length > 0 ? (
                      ...
                      <div className="p-8 text-center text-gray-500 text-sm">
                        Brak dodatkowych plików ogólnych.
                      </div>
                    )}

                    {/* 6. Rozwijana lista plików statycznych */}
                    {staticMembershipDocs && (
                      <CollapsibleFileCategory
                        category={staticMembershipDocs}
                      />
                    )}
                  </div>
                </div>
