...
// Zakładka „Wydarzenia" ukryta do czasu ukończenia modułu rejestracji (Faza 2b).
// Backend/API i strona /wydarzenia działają — tu tylko chowamy wejście w panelu.
...
export default function AdminDashboard() {
  ...
  const { handleStatusChange, modalStates } = useNotificationModals(
    submissions,
    setSubmissions,
  );
  ...
  const { declarations, activeSurveys, archivedSurveys } = useMemo(() => {
    const declarations = submissions.filter((s) =>
      ["DEKLARACJA_CZLONKOWSKA", "PATRONAT"].includes(s.formType),
    );
    const surveys = submissions.filter((s) =>
      ["ANKIETA_SPEDYTOR_ROKU", "MLODY_SPEDYTOR_ROKU"].includes(s.formType),
    );
  ...
  const handleArchiveToggle = async (submission, isArchived) => {
    ...
    setSubmissions((current) =>
      current.map((sub) =>
        sub.id === submission.id ? { ...sub, isArchived } : sub,
      ),
    );
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Zmiana statusu archiwum nie powiodła się.");
      toast.success(
        isArchived
          ? "Zgłoszenie zostało zarchiwizowane."
          : "Zgłoszenie zostało przywrócone.",
      );
    } catch (error) {
      console.error(error);
  ...
  const confirmDeleteAttachment = async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/submissions/${submissionId}/attachments/${attachmentId}`,
        {
          method: "DELETE",
        },
      );
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-[#005698] tracking-tight">
              Panel Administracyjny
            </h1>
            <p className="text-gray-600 mt-1">
              Przeglądaj i zarządzaj złożonymi deklaracjami.
            </p>
          </div>
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-700 hidden sm:block">
              Witaj,{" "}
              <strong className="text-[#005698]">
        ...
        <main className="bg-white rounded-lg shadow">
          {isLoading ? (
            ...
            <div>
              ...
              {EVENTS_ENABLED && activeTab === "events" && (
                <div className="p-4 sm:p-6">
                  <EventsManagement />
                </div>
              )}
