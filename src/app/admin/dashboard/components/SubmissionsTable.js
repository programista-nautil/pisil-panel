  ...
  if (!submissions || submissions.length === 0) {
    return (
      <p className="p-6 text-center text-gray-500">
        Brak zgłoszeń do wyświetlenia w tej kategorii.
      </p>
    );
  }

  const isSurveyType = (t) =>
    t === "ANKIETA_SPEDYTOR_ROKU" || t === "MLODY_SPEDYTOR_ROKU";
  const hasExpandableRows = submissions.some(
    (s) =>
      s.formType === "DEKLARACJA_CZLONKOWSKA" ||
      s.formType === "PATRONAT" ||
      isSurveyType(s.formType),
  );

  return (
    <div className="overflow-x-auto">
      <table className="table-fixed w-full min-w-[800px] text-sm text-left text-gray-500">
        <thead className="text-xs text-gray-700 uppercase bg-gray-50">
          <tr>
            {hasExpandableRows && (
              <th className="w-8 px-2 py-3" aria-label="Rozwiń" />
            )}
            ...
            <th
              scope="col"
              className="w-32 px-4 py-3 font-semibold whitespace-nowrap"
            >
              Data Złożenia
            </th>
            <th
              scope="col"
              className="w-36 px-4 py-3 font-semibold text-right whitespace-nowrap"
            >
              Akcje
            </th>
          </tr>
        ...
        <tbody>
          {submissions.map((submission) => {
            ...
            const isExpandable =
              ...
              isSurveyType(submission.formType);
            const clientFiles =
              submission.attachments?.filter(
                (att) => att.source === "CLIENT_UPLOAD",
              ) || [];
            const generatedFiles =
              submission.attachments?.filter(
                (att) => att.source === "GENERATED",
              ) || [];
              ...
              <Fragment key={submission.id}>
                ...
                <tr className="bg-white border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    {getFormTypeName(submission.formType)}
                  </td>
                  <td
                    className="px-4 py-4 font-medium text-gray-900 overflow-hidden"
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {submission.createdByAdmin && (
                        <span
                          title="Dodane ręcznie przez administratora"
                  ...
                  <td className="px-4 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      {onArchiveToggle && (
                        <button
                          onClick={() =>
                            onArchiveToggle(submission, !submission.isArchived)
                          }
                      ...
                      <button
                        ...
                        title={
                          submission.acceptanceNumber
                            ? "Archiwizuj zgłoszenie"
                            : "Usuń zgłoszenie trwale"
                        }
                      >
                        {submission.acceptanceNumber ? (
                          // Ikona Archiwizacji dla przyjętych
                          <ArchiveBoxArrowDownIcon className="h-5 w-5" />
                        ) : (
                          // Ikona Kosza dla roboczych
