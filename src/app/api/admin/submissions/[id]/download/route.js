...
export async function GET(request, { params }) {
	...
	try {
		...
		const fileBuffer = await downloadFileFromGCS(submission.filePath)

		// Ustaw nagłówki, aby przeglądarka pobrała plik
		const headers = new Headers()
		headers.set('Content-Type', 'application/pdf')
		const asciiFilename = submission.fileName.replace(/[^\x00-\x7F]/g, '_')
		const utf8Filename = encodeURIComponent(submission.fileName)
