...
const { sendBulkIdempotent } = require('./mailBatch')
...
const buildMessage = email => ({ to: email, subject: 't', html: 'h' })
...
test('dwa przebiegi tego samego zadania = jedna wysyłka na osobę', async () => {
	...
	const opcje = { scope: 'notify-members', refId: 'sub1', recipients: adresy(20), buildMessage, batchSize: 20 }

	const r1 = await sendBulkIdempotent(opcje, deps(prisma, sendToOne))
	expect(r1.sent).toBe(20)

	// Ponowienie (BullMQ „co najmniej raz") — wszystkie mają już znacznik.
	const r2 = await sendBulkIdempotent(opcje, deps(prisma, sendToOne))
...
test('awaria po 15 z 20: ponowienie dosyła TYLKO brakujące 5', async () => {
	...
	const recipients = adresy(20)
	const juzWyslane = recipients.slice(0, 15).map(email => `notify-members|sub1|${email}`)
	const prisma = fakePrisma(juzWyslane)
	const sendToOne = jest.fn().mockResolvedValue({})

	const r = await sendBulkIdempotent(
		{ scope: 'notify-members', refId: 'sub1', recipients, buildMessage, batchSize: 20 },
	...
	expect(dosłane).toEqual(recipients.slice(15).sort())
})

test('błąd wysyłki jednego adresu nie zabija partii; oznacza go jako BLAD', async () => {
	const recipients = adresy(5)
	const prisma = fakePrisma()
	const sendToOne = jest.fn().mockImplementation(async ({ to }) => {
		if (to === recipients[2]) throw new Error('550 zły adres')
	})

	const r = await sendBulkIdempotent(
		{ scope: 'notify-members', refId: 'sub1', recipients, buildMessage, batchSize: 5 },
		deps(prisma, sendToOne)
	)

	expect(r.sent).toBe(4) // pozostałe cztery poszły
	expect(r.errors).toEqual([{ email: recipients[2], error: '550 zły adres' }])
...
test('pacing: przerwa MIĘDZY partiami, nie po ostatniej', async () => {
	...
	await sendBulkIdempotent(
		{ scope: 's', refId: 'r', recipients: adresy(5), buildMessage, batchSize: 2, delayMs: 60000 },
		d
	)
